...
(defmacro defdomtest [name value-map & body]
 ...
 `(cemerick.cljs.test/deftest ~name
    (let [old-state# @minesweeper.state/app-state]
      ~@(for [[path v] value-map]
          (list 'reset! `(reagent.cursor/cur minesweeper.state/app-state ~path) v))
      (reagent.core/force-update-all)
      ~@body
      (minesweeper.state/reset-state! old-state#)
      (reagent.core/force-update-all))))
